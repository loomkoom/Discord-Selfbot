...
class Google:
    ...
    async def i(self, ctx, *, query):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("https://www.googleapis.com/customsearch/v1?q=" + urllib.parse.quote_plus(query) + "&start=" + '1' + "&key=" + os.environ['google_api_key'] + "&cx=" + os.environ['custom_search_engine'] + "&searchType=image") as resp:
                if resp.status != 200:
                    if not os.environ['google_api_key'] or not os.environ['custom_search_engine']:
