...
class Google:
    ...
    async def g(self, ctx, *, query):
        ...
        if not embed_perms(ctx.message):
            ...
            async with aiohttp.get("https://www.googleapis.com/customsearch/v1?q=" + query.replace(' ', '+') + "&start=" + '1' + "&key=" + os.environ['google_api_key'] + "&cx=" + config['custom_search_engine']) as resp:
    ...
    async def i(self, ctx, *, query):
        ...
        if query[0].isdigit():
            ...
            item = 0
        async with aiohttp.get("https://www.googleapis.com/customsearch/v1?q=" + query.replace(' ', '+') + "&start=" + '1' + "&key=" + os.environ['google_api_key'] + "&cx=" + config['custom_search_engine'] + "&searchType=image") as resp:
            if resp.status != 200:
